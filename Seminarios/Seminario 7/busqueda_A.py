...
class Nodo:
    ...
    def __lt__(self, estado) -> bool:
        return self.coste + self.heuristica < estado.heuristica + estado.coste
...
def busqueda_A() -> bool:
    ...
    while len(abiertos) != 0 and not objetivo:
        ...
        if not testObjetivo(actual.estado):
            if actual.crearHash() not in cerrados or cerrados[actual.crearHash()] > (actual.coste + actual.heuristica):
                sucesores = expandir(actual) 
                abiertos = abiertos + sucesores
                abiertos.sort()
                cerrados[actual.crearHash()] = actual.coste + actual.heuristica
        else:
            objetivo = True
    if objetivo:
        dispSolucion(actual)
